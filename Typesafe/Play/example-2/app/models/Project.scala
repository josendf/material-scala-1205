...
object Project {
  ...
  def renameFolder(folder: String, newName: String) {
    DB.withConnection { implicit connection =>
      SQL(
        """
          update project set folder = {newName} where folder = {name}
        """).on(
  ...
  def addMember(project: Long, user: String) {
    DB.withConnection { implicit connection =>
      SQL(
        """
          insert into project_member values({project}, {user})
        """).on(
        'project -> project,
        'user -> user
      ).executeUpdate()
    ...
    DB.withConnection { implicit connection =>
      SQL(
        """
          ...
          where project_id = {project} and user_email = {user}
        """
        ).on(
        'project -> project,
        'user -> user
  ...
  def create(project: Project, members: Seq[String]): Project = {
     DB.withTransaction { implicit connection =>
       ...
       val id: Long = project.id.getOrElse {
         SQL("SELECT LAST_INSERT_ID()").as(scalar[Long].single)
       }
       ...
       members.foreach { email =>
         SQL(
           """
             insert into project_member values ({id}, {email})
           """
           ).on(
           'id -> id, 'email -> email
         ).executeUpdate()

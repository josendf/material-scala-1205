...
object Task {
  ...
  def deleteInFolder(projectId: Long, folder: String) {
    DB.withConnection { implicit connection =>
      SQL(
        """
          delete from task
          where project = {project} and folder = {folder}
        """).on(
        'project -> projectId,
        'folder -> folder
      ).executeUpdate()
  ...
  def renameFolder(projectId: Long, folder: String, newName: String) {
    DB.withConnection { implicit connection =>
      SQL(
        """
          update task set folder = {newName}
          where folder = {name} and project = {project}
        """).on(
        'project -> projectId, 
        'name -> folder, 
        'newName -> newName
      ...
      SQL(
        """
          insert into task values (
            NULL, {title}, {done}, {dueDate}, {assignedTo}, {project}, {folder}
          )
        """
      ).on(
        'folder -> task.folder,

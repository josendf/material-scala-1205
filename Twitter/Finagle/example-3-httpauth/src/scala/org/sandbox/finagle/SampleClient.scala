...
/**
 * A somewhat advanced example of using Filters with Clients. Below,
 * HTTP 4xx and 5xx class requests are converted to
 * Exceptions. Additionally, two parallel requests are made and when
 * they both return (the two Futures are joined) the TCP connection(s)
 * are closed.
 */
object HttpClient {
  ...
  def main(args: Array[String]) {
    ...
    val clientWithoutErrorHandling: Service[HttpRequest, HttpResponse] =
        ClientBuilder()
           .codec(Http())
           .hosts(address)
    ...
    // When both request1 and request2 have completed, close
    // the TCP connection(s).
